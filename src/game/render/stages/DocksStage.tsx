...
function Container({
  ...
  return (
    <mesh position={position} castShadow receiveShadow>
      ...
      <meshStandardMaterial color={color} roughness={0.8} metalness={0.35} />
    </mesh>
  );
}
  ...
  return (
    <group>
      ...
      <mesh position={[0, -0.3, 0]} receiveShadow castShadow>
        ...
        <meshStandardMaterial color="#20272b" roughness={0.85} metalness={0.3} />
      </mesh>
      <ArenaFloor
        bounds={stage.bounds}
        color="#28323a"
      ...
      {[-bx + 0.4, 0, bx - 0.4].map((x) =>
        [-stage.bounds.z - 1, stage.bounds.z + 1].map((z) => (
          <mesh key={`${x}-${z}`} position={[x, -1.4, z]}>
            ...
            <meshStandardMaterial color="#171d21" roughness={0.9} />
          </mesh>
        )),
      )}
