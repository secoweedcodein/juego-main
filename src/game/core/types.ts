...
export interface InputIntent {
  ...
  dodge: boolean;
  crouch: boolean;
  /** tecla dedicada de bloqueo (adicional a retroceder) */
  block: boolean;
  /** ataques (FASE 2) */
  light: boolean;
  heavy: boolean;
...
export interface FighterState {
  ...
  /* --- FASE 9: knockdown / recuperación --- */
  /** ticks tumbado en el suelo (invulnerable, no puede actuar) */
  downTicks: number;
  /** ticks levantándose (no puede actuar) */
  getupTicks: number;
...
export interface MatchState {
  ...
  announce: string;
  /** daño acumulado causado por cada luchador (p1, p2), p/ estadísticas */
  damageDealt: [number, number];
  /** events consumidos por render/audio cada frame */
  events: HitEvent[];
